...
import { GET_USER, DELETE_ADDRESS, NEW_ADDRESS, UPDATE_USER, DELETE_USER } from './graphqlController';
		...
		const getUser = auth0_id => (
			<Query query={GET_USER} variables={{ auth0_id }}>
				{({ loading, error, data }) => {
					...
					address_id = data.user.address ? +data.user.address.address_id : "Please Log In";
					return (
						<>
			...
			<div className='user-profile-container'>
				...
				<div className='address-info'>
					{editingState ? (
						<React.Fragment>
							<p>
							{this.state.editingState === "add" || this.state.editingState === 'edit'? <>
								Address:{' '}
								<input
									type='text'
									name='address_one'
